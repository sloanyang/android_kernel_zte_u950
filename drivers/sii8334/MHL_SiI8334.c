...
static struct regulator *sii8334_vbus_power = NULL;

//add for preventing the phone into standby mode before MHL drivers do not work completely by liliguo 20121120
static struct wake_lock sii8334_wake_lock;  
...
///////////////////////////////////////////
 ...
 /* ZTE: added by liliguo for MHL VBUS POWER 20120712 */
static int sii8334_VBUS_POWERR_UP(void)
{
	int ret;
	
	if (!sii8334_vbus_power) {
		sii8334_vbus_power = regulator_get(NULL, "usb_vbus");
		if (IS_ERR_OR_NULL(sii8334_vbus_power)) {
			ret = PTR_ERR(sii8334_vbus_power);
			pr_err("MHL: couldn't get regulator usb_vbus\n");
			sii8334_vbus_power = NULL;
			return ret;
		}
	}
	
	ret = regulator_enable(sii8334_vbus_power);
	if (ret < 0) {
		pr_err("MHL: couldn't enable regulator usb_vbus\n");
		regulator_put(sii8334_vbus_power);
		sii8334_vbus_power = NULL;
		return ret;
	}
	return ret;
...
static int sii8334_VBUS_POWERR_DOWN(void)
{
	if (sii8334_vbus_power)
	{
		regulator_disable(sii8334_vbus_power);
		regulator_put(sii8334_vbus_power);
		sii8334_vbus_power = NULL;
	}
...
static irqreturn_t Sii8334_mhl_interrupt(int irq, void *dev_id)
{
	...
	//add for preventing the phone into standby mode before MHL drivers do not work completely by liliguo 20121120
	wake_lock_timeout(&sii8334_wake_lock, msecs_to_jiffies(5000));
	return IRQ_HANDLED;
}
#else
/*****************************************************************************/
...
{
	//add for preventing the phone into standby mode before MHL drivers do not work completely by liliguo 20121120
	wake_lock_init(&sii8334_wake_lock, WAKE_LOCK_SUSPEND, "sii8334_lock");
...
static void __exit mhl_Sii8334_exit(void)
{
	...
	//add for preventing the phone into standby mode before MHL drivers do not work completely by liliguo 20121120
	wake_lock_destroy(&sii8334_wake_lock);
