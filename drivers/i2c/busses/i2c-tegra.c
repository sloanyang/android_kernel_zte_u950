...
static int tegra_i2c_init(struct tegra_i2c_dev *i2c_dev)
{
    ...
    if (i2c_dev->i2c_is_suspended )
    {
    	printk("tegra_i2c_init:i2c_is_suspended\n");
    	return 0;
    }
	...
	struct i2c_msg *msg, enum msg_end_type end_state)
{
	...
	if (WARN_ON(ret == 0)) {
		...
		status = i2c_readl(i2c_dev, I2C_INT_STATUS);
		dev_err(i2c_dev->dev,
			"i2c transfer timed out(status=%d), addr 0x%04x, data 0x%02x\n",
			status, msg->addr, msg->buf[0]);
	...
	int num)
{
	...
	if (WARN_ON(i2c_dev->is_suspended)) {
		rt_mutex_unlock(&i2c_dev->dev_lock);
		return -EBUSY;
	}

	if ( i2c_dev->i2c_is_suspended ) {
		printk("i2c_is_suspended is true\n");
	    rt_mutex_unlock(&i2c_dev->dev_lock);
		return -EBUSY;
	}
...
static int tegra_i2c_resume_noirq(struct device *dev)
{
    ...
    i2c_dev->i2c_is_suspended = false;
	ret = tegra_i2c_init(i2c_dev);
	i2c_dev->i2c_is_suspended = true;
