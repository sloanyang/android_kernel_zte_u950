...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	if (type == EV_ABS) {
		if (state) {
			dev_info(&input->dev, "EV_ABS code=%d, value=%d\n", button->code, button->value);
			input_event(input, type, button->code, button->value);
		}
	} else {
		dev_info(&input->dev, "code=%d, value=%d\n", button->code, !!state);
...
static void gpio_keys_gpio_work_func(struct work_struct *work)
{
	...
	if (is_suspended) {
		Gpio_keys_state = 1;
		printk("gpio_keys_gpio_work_func is suspended\n");
		return;
	}

	if (Gpio_keys_state > 0 && bdata->button->code == KEY_POWER) {
		int state = (gpio_get_value_cansleep(bdata->button->gpio) ? 1 : 0)^ bdata->button->active_low;
		if ( !state)
			Gpio_keys_state = 0;
		printk("gpio_keys_gpio_work_func KEY_POWER is invalid\n");
		return;
	}
...
static int gpio_keys_resume(struct device *dev)
{
	...
	printk("---gpio_keys_resume---wakeup_key:%d\n", wakeup_key);
	is_suspended = 0;
	if (Gpio_keys_state || wakeup_key != KEY_POWER)
		Gpio_keys_state = 0;
	else
		Gpio_keys_state = 1;
