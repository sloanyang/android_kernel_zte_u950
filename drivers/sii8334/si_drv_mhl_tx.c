...
//#if Disconnected_USB_ID_with_CBUS_ID_As_defult
...
void CBUS_ID_Opened_AfterPlugedout( void )
{
	...
	TX_DEBUG_PRINT(("Drv_OTG: USB_ID should disconnected with CBUS_ID now.\n"));
...
static void	DetectRgndtoSetUSBID( void )
{
	...
	if((0x00 == reg99RGNDRange)||(0x02 == reg99RGNDRange))
		{
		TX_DEBUG_PRINT(("Drv: USB_ID should disconnected with CBUS_ID now.\n"));
		ForceUsbIdSwitchOpen();
		ENABLE_DISCOVERY
        //TX_DEBUG_PRINT(("Drv: ->D3  0x92:0x%02x  0x95:0x%02x\n",(int)ReadBytePage0(0x92),(int)ReadBytePage0(0x95)));
		//
		...
		TX_DEBUG_PRINT(("Drv: USB_ID should connected with CBUS_ID now.\n"));
