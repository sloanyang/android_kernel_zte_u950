...
//#define REQUEST_FIRMWARE_SUPPORTED                    1
...
#if 1// add by jiabf copy from 712
...
#define CMMB_SMS118X_MAX_STATS_NUM_CHANNELS				  10 
#define CMMB_STATS_ERR_HISTORY_LEN                      8

struct SmsMsgData_ST_TEST {
	struct SmsMsgHdr_ST xMsgHeader;
	u32 msgData[40];
};

struct SmsMsgData_LOOP_TEST {
	struct SmsMsgHdr_ST xMsgHeader;
	u32 msgData[1024];
};
...
typedef struct
{
	...
	s32  SNR;						//!< dB
	s32  RSSI;					//!< dBm
	s32  ErrorsCounter;			//!< Firmware errors
	...
	UINT32 IsBandwidth2Mhz;			//!< Bandwidth - 0 means 8 MHz, otherwise 2 MHz
	UINT32 ModemState;				//!< 0 - Acquisition, 1 - Locked

 	// Per Channel Information
 	UINT32	NumActiveChannels;			//!< Number of channels - including control
	UINT8	ErrorsHistory[CMMB_STATS_ERR_HISTORY_LEN];
	
 	CMMB_SMS118X_CHANNEL_STATS	ChannelsStatsArr[CMMB_SMS118X_MAX_STATS_NUM_CHANNELS];		//!< Channels info
} CMMB_SMS118X_STATISTICS;
#endif
...
#define DBG_ADV  2
 ...
 #define SMS_CMMB_DEBUG_SWITCH 1
