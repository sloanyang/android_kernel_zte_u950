...
#include "../../arch/arm/mach-tegra/include/mach/thermal.h"

extern struct tegra_thermal_data *therm;
...
static void therm_est_work_func(struct work_struct *work)
{
	...
	if (est->callback && ((est->cur_temp >= est->therm_est_hi_limit) ||
		...
		{
			if(therm->performance_sw)
			{
				printk("therm->performance_sw = %d\n",therm->performance_sw);
			}
			else
			{
				printk("therm->performance_sw = 0\n");
		est->callback(est->callback_data);
			}
		}
	else
		{
		printk("est->cur_temp is ok!\n");
	}
