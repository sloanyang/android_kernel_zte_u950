...
struct tegra_detect_int {
	int int_counter;
	unsigned long before_time;
	struct timer_list timer;
};

#define TEGRA_DETECT_COUNT_PERIOD 10
#define TEGRA_DETECT_TIME_THRESH 500
struct tegra_detect_int  tdi;
...
static void tdi_timer(unsigned long data)
{
	struct tegra_sdhci_host *tegra_host = (struct tegra_sdhci_host *)data;
...
static irqreturn_t carddetect_irq(int irq, void *data)
{
	...
	if (plat->mmc_data.status) {
		tegra_host->card_present = plat->mmc_data.status(mmc_dev(sdhost->mmc));
	}else{
		tegra_host->card_present = (gpio_get_value(plat->cd_gpio) == plat->cd_polarity);
	}
	...
	if(!tdi.int_counter){
		tdi.before_time = jiffies;
	}

	tdi.int_counter += 1;
	if(tdi.int_counter >=TEGRA_DETECT_COUNT_PERIOD){
		if(time_before(jiffies, tdi.before_time + msecs_to_jiffies(TEGRA_DETECT_TIME_THRESH))){
			printk("%s: SD card detect pin(%ld) occur the irq storm[time=%d]\n", mmc_hostname(sdhost->mmc), \
								plat->cd_gpio, jiffies-tdi.before_time);
			sdhost->mmc->caps = MMC_CAP_NEEDS_POLL;
			tegra_host->card_present = 0;
			mod_timer(&tdi.timer, jiffies+msecs_to_jiffies(500));
			disable_irq_nosync(gpio_to_irq(plat->cd_gpio));
		}
		tdi.int_counter = 0;
	}
...
static int __devinit sdhci_tegra_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(plat->cd_gpio)) {
		...
		tdi.int_counter = 0;
		tdi.before_time = 0;
		setup_timer(&tdi.timer, tdi_timer, (unsigned long)tegra_host);
	...
	if (!plat->mmc_data.built_in && !plat->mmc_data.single_power) {
	...
	if(plat->async_suspend)
		device_enable_async_suspend(mmc_dev(host->mmc));
