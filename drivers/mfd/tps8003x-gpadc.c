...
//modify lupeng
extern bool tps80031_init_flag;
...
static int tps80031_gpadc_suspended;
						...
						uint32_t channel)
{
	...
	#endif
    raw_channel_value =(raw_code * 1000 *5) >> (bits + 2) ;
    gain_error = tps80031_calib_tbl[channel].gain_error;
	offset_error = tps80031_calib_tbl[channel].offset_error;
    corrected_code = (raw_channel_value * SCALE - offset_error) / gain_error;
    //printk("[%s]CXG get channel_value=%d,raw_code=%d,raw_channel_value=%d,corrected_code=%d\n",__func__,channel_value,raw_code,raw_channel_value,corrected_code);
...
/*ZTE:add by caixiaoguang 20120914 for current when sleep++*/
void tps80031_gpadc_enable(void)
{
    int ret = 0;

	ret = tps80031_reg_write(the_gpadc, SLAVE_ID2, GPADC_CTRL, 0xef);
	if (ret < 0)
		return ret;
}
void tps80031_gpadc_disable(void)
{
    int ret = 0;
    
	ret = tps80031_reg_write(the_gpadc, SLAVE_ID2, GPADC_CTRL, 0x0);
	if (ret < 0)
		return ret;
...
int tps80031_gpadc_conversion(int channel_no)
{
	...
	if (tps80031_gpadc_suspended) {
		pr_err("%s: tps80031_gpadc_suspended!\n", __func__);
		return -EIO;
	}
...
static int __devinit tps80031_gpadc_probe(struct platform_device *pdev)
{
	...
	for (index = 0; index < TPS80031_GPADC_MAX_CHANNELS; index++) {
		...
		if (~scalar_bit_map & (1 << index)) {
			ret = tps80031_reg_read(gpadc, SLAVE_ID3,//origin is SLAVE_ID2
		...
		ret = tps80031_reg_read(gpadc, SLAVE_ID3, TPS80031_GPADC_TRIM5,//origin is SLAVE_ID2
							&l_delta1);
		if (ret < 0)
			goto err;
		ret = tps80031_reg_read(gpadc, SLAVE_ID3, TPS80031_GPADC_TRIM6,//origin is SLAVE_ID2
							&l_delta2);
		if (ret < 0)
			goto err;
		ret = tps80031_reg_read(gpadc, SLAVE_ID3, TPS80031_GPADC_TRIM19,//origin is SLAVE_ID2
							&h_delta2);
		if (ret < 0)
			goto err;
	...
	printk("*********tps80031_gpadc_probe***********");
	tps80031_init_flag = true;
	return 0;
err:
...
static int tps80031_gpadc_suspend(struct platform_device *pdev, pm_message_t state)
{
	tps80031_gpadc_suspended = 1;
	return 0;
}

static int tps80031_gpadc_resume(struct platform_device *pdev)
{
	tps80031_gpadc_suspended = 0;
	return 0;
}
...
static struct platform_driver tps80031_gpadc_driver = {
	...
	.suspend = tps80031_gpadc_suspend,
	.resume = tps80031_gpadc_resume,
	.driver		= {
		.name	= "tps80031-gpadc",
		.owner	= THIS_MODULE,
	},
