...
#define PMU_MSECURE    TEGRA_GPIO_PF7
...
extern void tps80031_set_usb_high_current(int high);
//static ssize_t set_usb_high_current_proc_write(struct file *filp, const char *buff, size_t len, loff_t *off)
			   ...
			   unsigned long count, void *data)
{
    ...
    if('1' == cmd )
        {
            tps80031_set_usb_high_current(1);
        }
    else if('0' == cmd )
        {
            tps80031_set_usb_high_current(0);
        }
    return 1;
}
...
{  
	struct proc_dir_entry *entry;        
       entry = create_proc_entry("driver/set_usb_high_current", 0666, NULL);     
   	if (entry)  
...
static int __devinit tps80031_rtc_probe(struct platform_device *pdev)
{
	...
	if (err) {
	  ...
	  gpio_request(PMU_MSECURE, "rtc_MSECURE");
      gpio_direction_output(PMU_MSECURE, 1);
      tegra_gpio_enable(PMU_MSECURE);
...
static ssize_t rtc_proc_read(struct file *filp,char *buff, size_t len, loff_t *off)
{
	...
	tps80031_read_regs(p_dev, 0x10, 6, &reg_val[0]);
	...
	tps80031_read_regs(p_dev, 0x0, 6, &reg_val[11]);
