...
{      
    ...
    if(enable)
        gpio_set_value(usb_port_ctrl, 0);
    else
        gpio_set_value(usb_port_ctrl, 1);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (status == 0) {
        ...
        if (value == 1) {
            ...
            //modem_download_enable(0);
            modem_power_enable(0);
            modem_usb3v3_enable(0);
            msleep(1000);
            modem_usb3v3_enable(1);
            modem_power_enable(1);
            //modem_download_enable(1);
            modem_power_key_set(1);
        } else {
            status = -EINVAL;
        }
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (status == 0) {
        ...
        if (value == 1) {
            ...
            //modem_download_enable(0);
            modem_power_enable(0);
            modem_reset_enable(0);
            modem_usb3v3_enable(0);
            msleep(500);
            modem_power_enable(1);
            modem_usb3v3_enable(1);
            modem_reset_enable(1);
            msleep(500);
            modem_reset_enable(0);
            //modem_download_enable(1);
            modem_power_key(1);
...
static int __devexit modem_control_remove(struct platform_device *pdev)
{
    ...
    gpio_free(usb_port_ctrl);
    gpio_free(modem_usb3v3_enb);
    gpio_free(modem_power_key);
    gpio_free(modem_download_en);
    return 0;
}
