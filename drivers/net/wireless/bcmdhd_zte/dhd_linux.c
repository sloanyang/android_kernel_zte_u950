...
dhd_open(struct net_device *net)
{
	...
	if (ifidx == 0) {
		...
		/* djt 2012.11.09 modified, begin*/
		{
			ret = wl_android_wifi_on(net);
			if (ret != 0)
			{
				DHD_ERROR(("wl_android_wifi_on: failed with code %d\n", ret));
				goto exit;
			}
		}
		/* djt 2012.11.09 modified, begin*/
...
exit:
	/* djt 2012.11.09 add, begin*/
	if (ret != 0 )
	{
		wl_android_wifi_off(net);
	}
	/* djt 2012.11.09 add, end*/
	DHD_OS_WAKE_UNLOCK(&dhd->pub);
	return ret;
}
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (!ret) {
        ...
        memcpy(dhd->mac.octet, ea_addr.octet, ETHER_ADDR_LEN);// djt 2011.01.09 add
	} else {
#endif /* GET_CUSTOM_MAC_ENABLE */
...
#endif /* defined(CONFIG_WIRELESS_EXT) */
//Modify for Softap-restart 0803 -
#if 0
