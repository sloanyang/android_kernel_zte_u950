...
/*ZTE: add by tengfei for led driver, 2012.5.29,++*/
#define TPS80031_LED_PWM_CTRL1 0xF4
#define TPS80031_LED_PWM_CTRL2 0xF5
/*ZTE: add by tengfei for led driver, 2012.5.29,--*/
/*ZTE: add by caixiaoguang 20120827 for pmu version++*/
...
/*ZTE: add by tengfei for led driver, 2012.5.29,++*/
int tps8003x_offred_verify(void)
{
	struct tps80031_client *tps = NULL;
	uint8_t reg_val = 0;
	int ret_status = -EINVAL;

	if(!tps80031_dev)
		goto err;

	tps = &tps80031_dev->tps_clients[SLAVE_ID2];

	ret_status = __tps80031_read(tps->client, TPS80031_LED_PWM_CTRL2, &reg_val);
	if(ret_status < 0)
		goto err;
...
EXPORT_SYMBOL_GPL(tps8003x_offred_verify);

int tps8003x_redlight_ctrl(int onoff, int dimming)
{
	struct tps80031_client *tps = NULL;
	uint8_t reg_val = 0;
	int ret_status = -EINVAL;
	int dimming_temp = dimming;
	static int once = 0;
	...
	if(0 == once)
	{
		...
		ret_status =__tps80031_write(tps->client, TPS80031_LED_PWM_CTRL2, reg_val);
		if(ret_status < 0)
			goto err;

		/*Step2: Select Power To Extpin.*/
		ret_status = __tps80031_read(tps->client, TPS80031_LED_PWM_CTRL2, &reg_val);
		if(ret_status < 0)
			goto err;
		...
		ret_status =__tps80031_write(tps->client, TPS80031_LED_PWM_CTRL1, (dimming_temp-1));
		if(ret_status < 0)
			goto err;
	...
	reg_val &= (~0x03);
	reg_val |= ((onoff) ? (0x01 << 0) : (0x02 << 0)); //Set Intensity
...
EXPORT_SYMBOL_GPL(tps8003x_redlight_ctrl);
/*ZTE: add by tengfei for led driver, 2012.5.29,--*/

/*ZTE: add by caixiaoguang for get pmu version 20120824++*/
int tps80031_zte_get_pmu_version(void)
{
    struct tps80031_client *tps = NULL;
	uint8_t reg_val = 0;
    char pmu_version[20] = {0};
	tps = &tps80031_dev->tps_clients[SLAVE_ID3];	
	__tps80031_read(tps->client, 0x87, &reg_val);
    printk("[%s]CXG get reg_val=%x\n",__func__,reg_val);
    sprintf(pmu_version, "0x%x", reg_val);
    prop_add("pm", "ver", pmu_version);
}
/*ZTE: add by caixiaoguang for get pmu version 20120824--*/
