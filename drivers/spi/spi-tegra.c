...
static int tegra_spi_clk_disable(struct device *dev);
static int tegra_spi_clk_enable(struct device *dev);
		    ...
		    bool is_single_xfer)
{
	...
	if (is_first_of_msg) {
		tegra_spi_clk_enable(&tspi->pdev->dev);
...
static int spi_tegra_setup(struct spi_device *spi)
{
	...
	tegra_spi_clk_disable(&tspi->pdev->dev);
	return 0;
}
...
fail_master_register:
	if (tspi->is_clkon_always)
	{
		tegra_spi_clk_disable(&pdev->dev);
	}
