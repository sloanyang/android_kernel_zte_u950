...
static int sensor_checkout_id(struct i2c_client *client, u8 *buf, int len)
{
	int err;
...
#ifdef CONFIG_PROJECT_U950	
	err = sensor_checkout_id(client, &sensor_id, 1);
	printk("the gyro I2C_ADD = 0x%x, sensor_id = 0x%x, err = %d", client->addr, sensor_id, err);
	if(WHOAMI_L3GD20 == sensor_id)
	{
		client->addr = 0x6B;
	}
#endif
